...
abstract class Payment extends TypeAgent
{
    ...
    public function isAvailable()
    {
        ...
        if (($this->guests && Isotope::getEnvironment()->hasMember()) || ($this->protected && !Isotope::getEnvironment()->hasMember()))
        {
            return false;
        }
        ...
        if ($this->protected)
        {
            ...
            if (!is_array($arrGroups) || empty($arrGroups) || !count(array_intersect($arrGroups, Isotope::getEnvironment()->getMemberGroups()))) // Can't use empty() because its an object property (using __get)
            {
                return false;
            }
        }
